...
def calc_gt_center(size_train, img_data, r=2, down=4, scale='h', offset=True):
    ...
    seman_map[1,:,:] = 1
    head_map = np.zeros((3, int(size_train[0]/down), int(size_train[1]/down)))
    head_map[1,:,:] = 1
    if len(igs) > 0:
        igs = igs/down
    ...
    if len(gts)>0:
        ...
        for ind in range(len(gts)):
            ...
            x3, y3, x4, y4 = int(x1+0.33*(x2-x1)), y1, int(x2-0.33*(x2-x1)), int(y1+0.15*(y2-y1))
            c_x2, c_y2 = int((gts[ind, 0] + gts[ind, 2]) / 2), int((y3+y4) / 2)
            dx2 = gaussian(x4-x3)
            dy2 = gaussian(y4-y3)
            gau_map2 = np.multiply(dy2, np.transpose(dx2))
            head_map[0, y3:y4, x3:x4] = np.maximum(head_map[0, y3:y4, x3:x4], gau_map2)
            head_map[1, y3:y4, x3:x4] = 1
            head_map[2, c_y2, c_x2] = 1
    ...
    if offset:
        return np.array(seman_map, dtype=np.float32),\
               np.array(scale_map, dtype=np.float32),np.array(offset_map, dtype=np.float32),np.array(head_map, dtype=np.float32),
    else:
