...
def prefetch_data(cfg, queue, sample_data, ped_data=None):
    ind = 0
    print("start prefetching data...")
    while True:
        try:
            data, ind = sample_data(cfg.dataset, ped_data, ind)  # ind可以保证每个batch取得数据不相同
            queue.put(data)
...
def init_parallel_jobs(cfg, queue, fn, ped_data=None):
    tasks = Process(target=prefetch_data, args=(cfg, queue, fn, ped_data))
    tasks.daemon = True
    tasks.start()
    return tasks
...
def train(logger, json_file, cfg, start_epoch=0):
    ...
    sample_module_city    = cfg.train_cfg.sample_module_city
    ...
    data_file   = "sample.{}".format(sample_module_city)
    sample_data = importlib.import_module(data_file).sample_data

    if cfg.train_cfg.cache_city:
        with open(cfg.train_cfg.cache_city, 'rb') as fid:
            ped_data = pickle.load(fid)
    length_dataset = len(ped_data)
