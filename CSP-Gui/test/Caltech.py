...
# **************************************
def kp_detection(cfg, nnet, img, result_dir, debug=False, gui = True, decode_func=kp_decode):
    if gui:
        gui_dir = os.path.join(result_dir, "gui")
    ...
    if gui:
        ...
        res_all = []
        if len(detections) > 0:
            f_res = np.repeat(cat_name, len(detections), axis=0).reshape((-1, 1))
            detections[:, [2, 3]] -= detections[:, [0, 1]]
            res_all += np.concatenate((f_res, detections), axis=-1).tolist()
        # gui_dir = os.path.join(gui, filepath[-7:])
        # cv2.imwrite(gui_dir, image)
        return image,res_all
    else:
        # if len(detections) > 0:
...
# *********************************
def testing(cfg, nnet, img, result_dir, debug=False, gui=True):
    return globals()[cfg.dataset.sampling_function](cfg, nnet, img, result_dir, debug=debug, gui=True)
