...
def train(logger, json_file, cfg, start_epoch=0):
    ...
    for epoch in range(start_epoch, num_epochs):
        for iteration in range(1, epoch_length + 1):
            ...
            training_loss,center_loss,scale_loss,offset_loss,head_loss,head_scale_loss,head_offset_loss = nnet.train(**training)

            loss.append(training_loss.item())
            loss_center.append(center_loss.item())
            loss_scale.append(scale_loss.item())
            loss_offset.append(offset_loss.item())
            loss_head.append(head_loss.item())
            loss_head_scale.append(head_scale_loss.item())
            loss_head_offset.append(head_offset_loss.item())

            if display and iteration % display == 0:
                loss = np.array(loss)
                loss_center = np.array(loss_center)
                loss_scale = np.array(loss_scale)
                loss_offset = np.array(loss_offset)
                loss_head = np.array(loss_head)
                loss_head_scale = np.array(loss_head_scale)
                loss_head_offset = np.array(loss_head_offset)
                logger.info("Epoch: {}/{}, loss_center: {:.5f}, loss_scale: {:.5f}, loss_offset: {:.5f}, loss_head_center: {:.5f}, loss_head_scale: {:.5f}, loss_head_offset: {:.5f}, loss_csp: {:.5f}".format(epoch+1, num_epochs, loss_center.sum() / display, loss_scale.sum() / display, loss_offset.sum() / display, loss_head.sum() / display, loss_head_scale.sum() / display, loss_head_offset.sum() / display, loss.sum() / display))
                text = {"Epoch": epoch+1,  "loss_center": round(loss_center.sum() / display, 5),  "loss_scale": round(loss_scale.sum() / display, 5),  "loss_offset": round(loss_offset.sum() / display, 5),  "loss_head_center": round(loss_head.sum() / display, 5), "loss_head_scale": round(loss_head_scale.sum() / display, 5),  "loss_head_offset": round(loss_head_offset.sum() / display, 5), "loss_csp": round(loss.sum() / display, 5)}
