...
class Network(nn.Module):
    ...
    def forward(self, xs, ys, **kwargs):
        ...
        loss,loss_center,loss_scale,loss_offset,loss_head_center,loss_head_scale,loss_head_offset  = self.loss(preds, ys, **kwargs)
        return loss,loss_center,loss_scale,loss_offset,loss_head_center,loss_head_scale,loss_head_offset
...
class NetworkFactory(object):
    ...
    def train(self, xs, ys, **kwargs):
        ...
        loss,loss_center,loss_scale,loss_offset,loss_head_center,loss_head_scale,loss_head_offset = self.network(xs, ys)
        loss = loss.mean()
        loss_center = loss_center.mean()
        loss_scale = loss_scale.mean()
        loss_offset = loss_offset.mean()
        loss_head_center = loss_head_center.mean()
        loss_head_scale = loss_head_scale.mean()
        loss_head_offset = loss_head_offset.mean()
