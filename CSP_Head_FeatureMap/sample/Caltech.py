...
def kp_detection(cfg, ped_data, k_ind, emp_data, ngt_ind):
    ...
    x_img_batch, y_seman_batch, y_height_batch, y_offset_batch, y_head_batch, y_head_height_batch, y_head_offset_batch = [], [], [], [], [], [], []
    ...
    for b_ind in range(0, batchsize_ped):
        ...
        y_seman, y_height, y_offset, y_head, y_head_height, y_head_offset = calc_gt_center(size_train, img_data_aug)
        ...
        y_head_batch.append(np.expand_dims(y_head, axis=0))
        y_head_height_batch.append(np.expand_dims(y_head_height, axis=0))
        y_head_offset_batch.append(np.expand_dims(y_head_offset, axis=0))
    ...
    y_head_batch = torch.from_numpy(np.concatenate(y_head_batch, axis=0))
    y_head_height_batch = torch.from_numpy(np.concatenate(y_head_height_batch, axis=0))
    y_head_offset_batch = torch.from_numpy(np.concatenate(y_head_offset_batch, axis=0))

    return {
        "xs": [x_img_batch],
        "ys": [y_seman_batch, y_height_batch, y_offset_batch, y_head_batch, y_head_height_batch, y_head_offset_batch]
    }, k_ind, ngt_ind
