...
def head_center_loss(pred, target, beta, gamma):
...
class HeadCenterLoss(nn.Module):
    def __init__(self,
                 beta=4,
                 gamma=2,
                 loss_weight=0.01):
        super(HeadCenterLoss, self).__init__()
        self.beta = beta
        self.gamma = gamma
        self.loss_weight = loss_weight

    def forward(self, pred, target):
        loss_cls = self.loss_weight*head_center_loss(pred, target, self.beta, self.gamma)
        return loss_cls
