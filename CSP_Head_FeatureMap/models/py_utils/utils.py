...
def parse_det_offset(Y, H, cfg, nms_algorithm, score=0.1, score_head=0.1, down=4):
    ...
    #         if hiou < miniou:
    #             miniou = hiou
    #     if miniou == 0 and head_box[4] > 0.5:
    #         w = (head_box[2] - head_box[0])/0.7
    #         h = (head_box[3] - head_box[1])/0.2
    #         x1 = head_box[0] - 0.15*w
    #         y1 = head_box[1]
    #         x2 = head_box[2] + 0.15*w
    #         y2 = head_box[1] + h
    #         s = head_box[4]
    #         bboxs.append([min(x1,0), y1, min(x2, cfg.dataset.size_test[1]), min(y2, cfg.dataset.size_test[0]), s])
