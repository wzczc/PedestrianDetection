...
def kp_decode(nnet, images, **kwargs):
    detections, head_detections = nnet.test([images], **kwargs)
    return detections, head_detections


def kp_detection(cfg, nnet, result_dir, debug, decode_func=kp_decode):
    if debug:
    ...
    for ind in tqdm(range(0, num_images), ncols=80, desc="locating kps"):
        ...
        detections, head_detections = decode_func(nnet, image, **img_meta)
        t2 = time.time()
        t = t2 - t1
        ...
        if debug:
            ...
            # color2 = color2.astype(np.int32).tolist()
            ...
            for head_bbox in head_detections:
                head_bbox = head_bbox[0:4].astype(np.int32)
                if head_bbox[1] - cat_size[1] - 2 < 0:
                    cv2.rectangle(image,
                                  (head_bbox[0], head_bbox[3] + 2),
                                  (head_bbox[0] + cat_size[0], head_bbox[3] + cat_size[1] + 2),
                                  color2, -1
                                  )
                    cv2.putText(image, 'head',
                                (head_bbox[0], head_bbox[3] + cat_size[1] + 2),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), thickness=1
                                )
                else:
                    cv2.rectangle(image,
                                  (head_bbox[0], head_bbox[3] - cat_size[1] - 2),
                                  (head_bbox[0] + cat_size[0], head_bbox[3] - 2),
                                  color2, -1
                                  )
                    cv2.putText(image, 'head',
                                (head_bbox[0], head_bbox[3] - 2),
            ...
            if len(detections) > 0:
                ...
                with open(debug_txt,"a") as f:  
                    np.savetxt(f, np.array(res_all), fmt='%6f')
                f.close()
        else:
            if len(detections) > 0:
                f_res = np.repeat(frame_number, len(detections), axis=0).reshape((-1, 1))
...
def testing(cfg, nnet, result_dir, debug):
